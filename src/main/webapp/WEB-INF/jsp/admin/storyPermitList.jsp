...
<body>
    ...
    <section class="all_wrap">
        ...
        <div class="container">
            ...
            <div class="clearfix">
                <p class="small_tit"><span class="tit_icon"></span>등록글 전체 보기</p>
                <hr />
                <select name="" onchange="" class="all_select" style="float:right">
                    <option>전체 목록</option>
                    <option value="">반려 목록</option>
                    <option value="" selected>등록 목록</option>
                </select>
            </div>
                ...
                <div class="m-up-down text-center d-flex justify-content-center align-items-center paging_btn"
                    ...
                    <c:if test="${prev}">
                        <span class="paging_prev">
                            <c:if test="${select == 1}">
                                [ <a href="${pageContext.request.contextPath}/storyPermitList.do?num=1">이전</a>
                                ]
                            </c:if>

                            <c:if test="${select > 1}">
                                [ <a
                                    href="${pageContext.request.contextPath}/storyPermitList.do?num=${select - 1}">이전</a>
                                ]
                            </c:if>
                        </span>
                    </c:if>
                    ...
                    <c:forEach begin="${startPageNum}" end="${endPageNum}" var="num">
                        <span class="paging_num">
                            <c:if test="${select != num}">
                                <a href="${pageContext.request.contextPath}/storyPermitList.do?num=${num}">${num}</a>
                            </c:if>
                        </span>
                    ...
                    <c:if test="${next}">
                        <span class="paging_next">
                            <c:if test="${select == 1}">
                                [ <a href="${pageContext.request.contextPath}/storyPermitList.do?num=1">다음</a>
                                ]
                            </c:if>

                            <c:if test="${select > 1}">
                                [ <a
                                    href="${pageContext.request.contextPath}/storyPermitList.do?num=${select + 1}">다음</a>
                                ]
                            </c:if>
                        </span>
                    </c:if>
